...
#chanefes for the appmod
# --- Setup and Initialization ---



# API Keys - Ensure these are set as Streamlit secrets or environment variables
...
def initialize_legal_rag(selected_model_option):
    ...
    if provider == "openai":
        ...
        model_instance = ChatOpenAI(model_name=model_name, temperature=0)

    elif provider == "groq":

        os.environ["GROQ_API_KEY"] = GROQ_API_KEY # Ensure API key is set

        model_instance = ChatGroq(api_key=GROQ_API_KEY, model_name=model_name, temperature=0.1)

    elif provider == "huggingface_hub":

        os.environ["HUGGINGFACEHUB_API_TOKEN"] = HUGGINGFACEHUB_API_TOKEN # Ensure API key is set

        model_instance = ChatHuggingFace(repo_id=model_name, model_kwargs={"temperature": 0.1, "max_new_tokens": 500}) # Example params, adjust as needed
